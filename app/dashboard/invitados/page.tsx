...
export default async function InvitadosPage() {
  ...
  const [{ data: responses }, { data: menuOptions }, { data: expectedGuests }] = await Promise.all([
    supabase
      .from('rsvp_responses')
      .select('id, guest_name, attendance, adults_count, adult_names, adult_menus, has_children, children_count, children_names, children_menus, bus_option, bus_outbound, bus_return, allergies, song_request, message, submitted_at')
